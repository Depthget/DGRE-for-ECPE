...
class IERGCN(nn.Module):
    ...
    def forward(self, couples_pos_emo, doc_sents_h):
        batch_size, doc_sen_len, _ = doc_sents_h.size()
        for i in range(batch_size):
            len_sen_ = doc_sen_len
            p, e, c, c_u, c_v = self.dgl_adj_matrix_construction(len_sen_, self.K)
            emotion = doc_sents_h[i]
            cause = doc_sents_h[i]
            pair = couples_pos_emo[i]

            G = self.pos_build_graph(p, e, c, c_u, c_v, emotion, cause, pair)
            ...
            for i_r_gcn_layer, rgcn_layer in enumerate(self.rgcn_layer_stack):
                pred_dict = rgcn_layer(G, {'emotion': emotion_feats, 'cause': cause_feats, 'pair': pair_feats})
    ...
    def pos_build_graph(self, p, e, c, c_u, c_v, emotion, cause, pair):
        G = dgl.heterograph({
    ...
    def dgl_adj_matrix_construction(self, seq_len, k):
        ...
        pair_idx = torch.tensor(np.array(list(map(lambda x: -k <= x <= k, pos.tolist())), dtype=np.int).reshape(seq_len, seq_len).tolist())
        cau_gcn_len_idx = torch.tensor(np.array(list(map(lambda y: -you_want_gcn_len_cau <= y <= you_want_gcn_len_cau, pos.tolist())), dtype=np.int).reshape(seq_len, seq_len).tolist())
        p, e, c, c_u, c_v = [], [], [], [], []
        r, prs_tmp = 0, 0
