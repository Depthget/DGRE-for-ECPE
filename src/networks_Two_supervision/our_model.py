...
class Network(nn.Module):
    ...
    def forward(self, bert_token_b, bert_segment_b, bert_masks_b, bert_clause_b, doc_len, adj):
        ...
        pred_emo = self.pred_e_layer(doc_sents_h)
        couples_pos_emo, emo_cau_pos = self.PG(doc_sents_h, pred_emo)
        couples_pred = self.iergcn(couples_pos_emo, doc_sents_h)
    ...
    def loss_pre_emo(self, pred_emo, y_emotions_label_b, y_mask):
        ...
        y_mask = torch.cat([y_mask] * self.tagset_size, dim=-1)
...
class Pair_Generate(nn.Module):
    ...
    def forward(self, doc_sents_h, pred_emo):
        ...
        couples_h, rel_pos, emo_cau_pos = self.couple_generator(doc_sents_h, self.K, emo_lab_embedding)
    ...
    def couple_generator(self, H, k, emo_lab_embedding):
