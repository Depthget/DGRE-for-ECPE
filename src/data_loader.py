...
def build_train_data(configs, fold_id, shuffle=True):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=configs.batch_size,
                                               shuffle=shuffle, drop_last=True, collate_fn=bert_batch_preprocessing)
    return train_loader
...
def build_inference_data(configs, fold_id, data_type):
    ...
    data_loader = torch.utils.data.DataLoader(dataset=dataset, batch_size=configs.batch_size,
                                              shuffle=False, drop_last=True, collate_fn=bert_batch_preprocessing)
    return data_loader
...
class MyDataset(Dataset):
    def __init__(self, configs, fold_id, data_type, data_dir=DATA_DIR):
        ...
        self.y_emotions_list, \
        self.y_emotions_label_list, \
        self.y_causes_list, \
        self.doc_len_list, \
        self.doc_id_list, \
        self.bert_token_idx_list, \
        self.bert_clause_idx_list, \
        self.bert_segments_idx_list, \
    ...
    def __getitem__(self, idx):
        doc_couples, y_emotions, y_emotions_label, y_causes = self.doc_couples_list[idx], self.y_emotions_list[idx], self.y_emotions_label_list[idx], self.y_causes_list[idx]
        ...
        if bert_token_lens > 512:
            ...
            y_emotions_label, \
            y_causes,\
            doc_len = self.token_trunk(bert_token_idx, bert_clause_idx, bert_segments_idx, bert_token_lens, doc_couples, y_emotions, y_emotions_label, y_causes, doc_len)
        ...
        return doc_couples, y_emotions, y_emotions_label, y_causes, doc_len, doc_id, bert_token_idx, bert_segments_idx, bert_clause_idx, bert_token_lens
    ...
    def read_data_file(self, data_type):
        ...
        emo_to_idx = {'null': 0, 'happiness': 1, 'happiness&happiness': 1,
                      'sadness': 2, 'sadness&sadness': 2, 'happiness&sadness': 2, 'anger&sadness': 2,
                      'anger': 3, 'anger&anger': 3, 'disgust&anger': 3,
                      'disgust': 4, 'disgust&disgust': 4, 'sadness&disgust': 4,
                      'surprise': 5,
                      'fear': 6, 'fear&fear': 6, 'happiness&fear': 6, 'disgust&fear': 6}
        doc_id_list = []
        doc_len_list = []
        doc_couples_list = []
        y_emotions_label_list = []
        ...
        for doc in data_list:
            ...
            emo_list_ = [[0] * self.tagset_size]
            emotion_list = emo_list_ * doc_len
            for i in range(len(doc_emotions)):
                emot_label = []
                emotion_clauses = doc_clauses[doc_emotions[i] - 1]
                emo = emotion_clauses['emotion_category']
                emo_l = emo_to_idx[emo]
                emot_label.append(emo_l)
                c_id = int(emotion_clauses['clause_id']) - 1
                for j in range(doc_len):
                    if j == c_id:
                        e_list_ = []
                        for k in range(self.tagset_size):
                            e_l_ = int(k in emot_label)
                            e_list_.append(e_l_)
                        emotion_list[j] = e_list_
            ...
            y_emotions_label_list.append(emotion_list)
            y_causes_list.append(y_causes)
        return doc_couples_list, y_emotions_list, y_emotions_label_list, y_causes_list, doc_len_list, doc_id_list, bert_token_idx_list, bert_clause_idx_list, bert_segments_idx_list, bert_token_lens_list

    def token_trunk(self, bert_token_idx, bert_clause_idx, bert_segments_idx, bert_token_lens, doc_couples, y_emotions, y_emotions_label, y_causes, doc_len):
        ...
        if emotion > doc_len / 2 and cause > doc_len / 2:
            ...
            while True:
                ...
                if len(temp_bert_token_idx) <= 512:
                    ...
                    y_emotions_label = y_emotions_label[i:]
                    y_causes = y_causes[i:]
                    doc_len = doc_len - i
                    break
                i = i + 1
        ...
        if emotion < doc_len / 2 and cause < doc_len / 2:
            ...
            while True:
                ...
                if len(temp_bert_token_idx) <= 512:
                    ...
                    break
                i = i - 1
        return bert_token_idx, bert_clause_idx, bert_segments_idx, bert_token_lens, doc_couples, y_emotions, y_emotions_label, y_causes, doc_len
...
def bert_batch_preprocessing(batch):
    ...
    bert_segment_b, \
    bert_clause_b, \
    bert_token_lens_b = zip(*batch)


    y_mask_b, y_emotions_b, y_emotions_label_b, y_causes_b = pad_docs(doc_len_b, y_emotions_b, y_emotions_label_b, y_causes_b)
    ...
    bert_masks_b = np.zeros([bsz, max_len], dtype=float)
    ...
    return np.array(doc_len_b), \
           np.array(adj_b), \
           np.array(y_emotions_b), \
           np.array(y_emotions_label_b), \
           np.array(y_causes_b), \
           np.array(y_mask_b), \
           doc_couples_b,\
           doc_id_b, \
...
def pad_docs(doc_len_b, y_emotions_b, y_emotions_label_b, y_causes_b):
    max_doc_len = max(doc_len_b)
    y_mask_b, y_emotions_b_, y_emotions_label_b_, y_causes_b_ = [], [], [], []
    for y_emotions, y_emotions_label, y_causes in zip(y_emotions_b, y_emotions_label_b, y_causes_b):
        y_emotions_ = pad_list(y_emotions, max_doc_len, -1)
        y_emotions_label_ = pad_list_(y_emotions_label, max_doc_len, -1, configs)
        ...
        y_emotions_label_b_.append(y_emotions_label_)
        y_causes_b_.append(y_causes_)
    return y_mask_b, y_emotions_b_, y_emotions_label_b_, y_causes_b_
...
def pad_list_(element_list, max_len, pad_mark, configs):
    element_list_pad = element_list[:]
    pad_mark_list = [[pad_mark] *configs.tagset_size]* (max_len - len(element_list))
    element_list_pad.extend(pad_mark_list)
    return element_list_pad