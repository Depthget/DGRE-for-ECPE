...
def main(configs, fold_id):
    ...
    params_rest = list(model.gnn.parameters()) + list(model.PG.parameters()) + list(model.pred_e_layer.parameters()) + list(model.pred.parameters()) + list(model.iergcn.parameters())
    assert sum([param.nelement() for param in params]) == sum([param.nelement() for param in params_bert]) + sum([param.nelement() for param in params_rest])
    ...
    for epoch in range(1, configs.epochs+1):
        for train_step, batch in enumerate(train_loader, 1):
            model.train()
            doc_len_b, adj_b, y_emotions_b, y_emotions_label_b, y_causes_b, y_mask_b, doc_couples_b, doc_id_b, bert_token_b, bert_segment_b, bert_masks_b, bert_clause_b = batch
            couples_pred, emo_cau_pos, pred_e, pred_emo, pred_c = model(bert_token_b, bert_segment_b, bert_masks_b, bert_clause_b, doc_len_b, adj_b)
            loss_emo = model.loss_pre_emo(pred_emo, y_emotions_label_b, y_mask_b)
            ...
            loss = loss_couple + loss_emo + loss_e + loss_c
...
def inference_one_epoch(configs, batches, model):
    ...
    for batch in batches:
        doc_couples, doc_couples_pred, doc_id_b = inference_one_batch(configs, batch, model)
